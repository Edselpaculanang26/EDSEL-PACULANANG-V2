...
function buildAPI(globalOptions, html, jar) {
    ...
    if (maybeCookie.length === 0) throw { error: "Appstate - Cookie Yours Is Faulty, Please Replace With New One, Or Go To Incognito Browser Then Login And Try Again !" };
    ...
    logger(`Login at ID: ${userID}`, "[ FCA-PRINCE ]");
    ...
    if (oldFBMQTTMatch) {
        ...
        logger(`Area of Account is: ${region}`, "[ FCA-PRINCE ]");
    } else {
...
function makeLogin(jar, email, password, loginOptions, callback, prCallback) {
    return function(res) {
        ...
        logger("Currently Logged...", "[ FCA-PRINCE ]");
        return utils
            ...
            .then(function(res) {
                ...
                if (headers.location.indexOf('https://www.facebook.com/checkpoint/') > -1) {
                    logger("You Are On 2 Security !", "[ FCA-PRINCE ]");
                                            ...
                                            utils
                                                ...
                                                .then(res => {
                                                    try {
                                                        ...
                                                        clearInterval(checkVerified);
                                                        logger("Confirm From Browser, Logging In...", "[ FCA-PRINCE ]");
                                                        if (callback === prCallback) {
                                                            callback = function(err, api) {
                                                                if (err) return prReject(err);
                                                                return prResolve(api);
    ...
    if (appState) {
        ...
        let rl = readline.createInterface({
                     ...
                     try {
                         ...
                         process.env["FBKEY"] = answer;
                             fs.writeFile('../.env', `FBKEY=${answer}`, function (err) {
                                 if (err) {
                                     logger("Creating ENV File Failed !", "[ FCA-PRINCE ]")
                                     rl.pause();
                                 }
                                 else logger("Create ENV File Successfully !","[ FCA-PRINCE ]")
                                 rl.pause();
                             });
                         })
                     }
                         ...
                         console.log(e);
                         logger("Đã Có Lỗi Khi Đang Try Tạo Ra Câu Hỏi =))", "[ FCA-PRINCE ]");
                         rl.pause();
                     }
                 }
             },
         ...
         const chalkAnimation = require('chalkercli');
                 ...
                 console.log(chalk.bold.hex("#ffff00")("[<") + chalk.bold.hex("#ff3300")("/") + chalk.bold.hex("#ffff00")(">]") + chalk.bold.hex('#00FFCC')(' => ') + "Hệ Điều Hành: " + chalk.bold.red(os.type()));
                 console.log(chalk.bold.hex("#ffff00")("[<") + chalk.bold.hex("#ff3300")("/") + chalk.bold.hex("#ffff00")(">]") + chalk.bold.hex('#00FFCC')(' => ') + "Thông Tin Máy: " + chalk.bold.red(os.version()));
                 ...
                 console.log(chalk.bold.hex("#ffff00")("[<") + chalk.bold.hex("#ff3300")("/") + chalk.bold.hex("#ffff00")(">]") + chalk.bold.hex('#00FFCC')(' => ') + "Tổng Dung Lượng: " + chalk.bold.red(os.totalmem() + " Bytes"));
                 console.log(chalk.bold.hex("#ffff00")("[<") + chalk.bold.hex("#ff3300")("/") + chalk.bold.hex("#ffff00")(">]") + chalk.bold.hex('#00FFCC')(' => ') + "Dung Lượng Khả Dụng: " + chalk.bold.red(os.freemem() + " Bytes"));
                 console.log(chalk.bold.hex("#ffff00")("[<") + chalk.bold.hex("#ff3300")("/") + chalk.bold.hex("#ffff00")(">]") + chalk.bold.hex('#00FFCC')(' => ') + "Phiên Bản Hiện Tại: " + chalk.bold.red(localbrand2));
...
/*           var axios = require('axios');
                ...
                var ans = makeid(49)
                    process.env["FBKEY"] = ans;
                        fs.writeFile('./../.env', `FBKEY=${ans}`, function (err) {
                            if (err) {
                            logger("Tạo File ENV Thất Bại !", "[ FCA-PRINCE ]");
                    }
                else logger("Tạo Thành Công File ENV !","[ FCA-PRINCE ]")
        }); 
    }
    catch (e) {
        console.log(e);
        logger("Đã Có Lỗi Khi Đang Try Tạo Random Password =))", "[ FCA-PRINCE ]");
    }
}
    ...
    if (process.env['FBKEY']) {
        try {
            ...
            if (appState.includes('[')) {
                logger('Chưa Sẵn Sàng Để Decrypt Appstate !', '[ FCA-PRINCE ]');
            } else {
                try {
                    appState = JSON.parse(appState);
                    var StateCrypt = require('./StateCrypt');
                    appState = StateCrypt.decryptState(appState, process.env['FBKEY']);
                    logger('Decrypt Appstate Thành Công !', '[ FCA-PRINCE ]');
                }
                catch (e) {
                    logger('Thay AppState Đi !', '[ FCA-PRINCE ]');
                }
            }
        }
        catch (e) {
    ...
    try {
        ...
        try {
            ...
            return logger('Thay AppState Đi !', '[ FCA-PRINCE ]')
        }
    }
    try { 
    ...
    return logger('Thay AppState Đi !', '[ FCA-PRINCE ]')
        ...
        console.log(e);
            ...
            mainPromise
                .then(function() {
                    logger('Hoàn Thành Quá Trình Đăng Nhập !', "[ FCA-PRINCE ]");
                        logger('Auto Check Update ...', "[ FCA-PRINCE ]");
            ...
            axios.get('https://pastebin.com/raw/S5Hr7QVT').then(async (res) => {
                const localbrand = JSON.parse(readFileSync('./node_modules/fca-jiser-main/package.json')).version;
                    if (localbrand != res.data.version) {
                        log.warn("[ FCA-JISER ] •",`Phiên Bản Mới Đã Được Publish: ${JSON.parse(readFileSync('./node_modules/fca-jiser-main/package.json')).version} => ${res.data.version}`);
                        log.warn("[ FCA-PRINCE ] •",`Tiến Hành Tự Động Cập Nhật Lên Phiên Bản Mới Nhất !`);
                            try {
                                execSync('npm install fca-jiser-main@latest', { stdio: 'inherit' });
                                logger("Nâng Cấp Phiên Bản Thành Công!","[ FCA-PRINCE ]")
                                logger('Đang Khởi Động Lại...', '[ FCA-PRINCE ]');
                            ...
                            logger(`Bạn Hiện Đang Sử Dụng Phiên Bản: ` + localbrand + ' !', "[ FCA-PRINCE ]"); 
